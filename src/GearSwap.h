...
namespace CIGAR
{
	...
	// rating as the inventory shows it, tempering and perks included; empty parts are left alone).
	// A worn enchanted piece is never offered a replacement (the user, 2026-09-25). Taking it is
	// never a crime. docs/029-si-leftovers.md.
	...
	private:
		...
		static float Rating(RE::PlayerCharacter* a_player, RE::TESObjectARMO* a_armor, RE::InventoryEntryData* a_entry);
		static bool SameClass(const RE::TESObjectARMO* a_a, const RE::TESObjectARMO* a_b);
		void Consider(RE::PlayerCharacter* a_player, const RE::TESObjectREFR::InventoryItemMap& a_mine,
