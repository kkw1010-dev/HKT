...
A module that is switched off skips `Tick()`, and its prompts are taken off the
screen at once (`Prompts::WithdrawAll`). `OnGameLoaded()` still runs, so
...
New modules need nothing extra. They appear in the panel through `Modules()`,
and `Panel.cpp` already has Korean labels for `BaboKey` and `LockOn`. After
merging, add the panel to `README.md` (layout, diagnostics) and `HANDOFF.md`.
