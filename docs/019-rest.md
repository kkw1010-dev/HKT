...
Pass time, as the user specified: time speeds up only while a key is held, never
by itself and never through the Wait menu (the reverted `6dd9a7d` opened it).

- 시간 보내기 (누르고 있기) appears as soon as a pose is entered, in any pose; the
  user wanted it at once, since a double press
...
- While the key is down the game clock's timescale global climbs from x1 to x60
  ...
  switched off put it back at once. x60 and 3 s are my choices. At the vanilla
  timescale 20, x60 is 20 game minutes per real second.
...
- Vanilla animation events to the player: `IdleSitCrossLeggedEnter`,
  `IdleLayDownEnter`; getting up with `IdleChairExitStart`, falling back to `IdleStop` and
  `IdleForceDefaultState`.
- The tags `idleChairSitting` and `tailLayDown` tell that the pose is reached (pass
  time counts from there).
- 1 s of standing idle before the prompts.

## Ledge sit

On an edge the player sits with the legs hanging (`IdleSitLedgeEnter`). On 앉기, rays on the line-of-sight layer, in the player's own collision group so
...
Lean events: `IdleWallLeanStart`, `IdleLeanTableEnter` and `IdleRailLeanEnter`; getting up
from the rail sends `IdleRailLeanExit`.
