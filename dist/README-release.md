...
3. **세부 설정** — 프롬프트가 뜨기 시작하는 기준값입니다. 물약의 비율, 먹기의 허기 단계, 용변 수치,
   무기 전환 거리, 유술 거리, 침대·옷장 유효 거리 등.

바꾼 값은 `SKSE/Plugins/CIGAR.json`에 저장됩니다.

## Streamlined Interactions와 함께 쓸 때

CIGAR 2.0은 Streamlined Interactions(SI)가 하던 일을 모두 대신합니다. **SI는 끄는 것을 권장합니다.**
둘을 같이 켜 두면 같은 행동의 프롬프트가 겹치고, 한 키에 두 동작이 걸릴 수 있습니다.

CIGAR가 일부러 하지 않는 SI 기능: 불빛 만들기, 도끼·곡괭이 자동 교체, 의자에서 먹기, 주변 정리하기, 킬무브 프롬프트
(처형은 CIGAR의 처형 프롬프트가 맡습니다).
