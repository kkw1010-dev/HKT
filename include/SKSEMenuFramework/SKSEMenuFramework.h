...
// Vendored from Thiago099/SKSE-Menu-Framework-2-Example @ cd1f277 (MIT, see LICENSE).
// CIGAR patch: upstream resolves the module handle during static initialisation. SKSE loads
// CIGAR.dll before SKSEMenuFramework.dll, so that handle would stay null and every call would
// silently do nothing. The handle is looked up when a function is first resolved instead.
inline HMODULE SKSEMenuFramework_Module() { return GetModuleHandleW(L"SKSEMenuFramework"); }
#define menuFramework SKSEMenuFramework_Module()
